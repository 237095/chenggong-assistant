...
def generate_verification_code(phone):
    ...
    st.session_state.verification_code[phone] = {
        "code": code,
...
def verify_code(phone, code):
    """验证验证码"""
    if phone not in st.session_state.verification_code:
        return False
    
    code_data = st.session_state.verification_code[phone]
    if datetime.now() > code_data["expires_at"]:
        del st.session_state.verification_code[phone]
        return False
        ...
        with col2:
            if st.button("获取验证码", key="get_code"):
                if phone and len(phone) == 11 and phone.isdigit():
                    ...
                    st.success(f"验证码：{code}")
                else:
                    st.error("请输入正确的手机号")
        
        if st.button("登 录", key="student_login_btn"):
            student_login(phone, verification_code)
            st.rerun()
